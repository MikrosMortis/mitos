$("document").ready(function () {
  ...
  function skipToEnd(message) {
    ...
    const error =
      currentLanguage === "greek"
        ? "Λυπούμαστε αλλά δεν δικαιούστε την έκδοση βεβαίωσης ανεργίας!"
        : "We are sorry, but you are not entitled to an unemployment certificate!";
  ...
  function retrieveAnswers() {
    ...
    if (allAnswers[3] === "2" || allAnswers[3] === "3") {
      getEvidencesById(2);
    }
    if (allAnswers[3] === "1") {
      getEvidencesById(3);
      currentLanguage === "greek"
      ? setResult(
        "Θα πρέπει να μεταβείτε στο ΚΕΠ για την έκδοση της βεβαίωσης ανεργίας.Θα πρέπει να είστε εγγεγραμμένος/η για τουλάχιστον ένα δίμηνο πριν την υποβολή της αίτησης,να είστε ανασφάλιστος/η και να μην σπουδάζετε."
      )
    : setResult(
        "You should go to the KEP to issue the unemployment certificate.You must be registered for at least two months prior to applying, be uninsured and not studying."
      );
    }
   /* if (allAnswers[2] === "1" && allAnswers[4] === "2") {
      getEvidencesById(6);
  ...
  $("#nextQuestion").click(function () {
    if ($(".govgr-radios__input").is(":checked")) {
      ...
      if (currentQuestion === 0 && selectedRadioButtonIndex === 2) {
        currentQuestion = -1;
        currentLanguage === "greek" ? skipToEnd("Θα πρέπει να είστε εγγεγραμμένος άνεργος για την έκδοση της βεβαίωσης.") : skipToEnd("You must be registered unemployed for the certificate to be issued.");
        ...
        currentQuestion = -1;
        currentLanguage === "greek" ? skipToEnd("Θα πρέπει να διαθέτετε κωδικούς ΔΥΠΑ. Αν είστε μέχρι 29 ετών απευθηνθείτε στο ΚΕΠ.") : skipToEnd("You should have DYPA codes.");
      } else if (currentQuestion === 4 && selectedRadioButtonIndex === 2) {
        currentQuestion = -1;
        currentLanguage === "greek" ? skipToEnd("Θα πρέπει να είστε άνεργος για τουλάχιστον ένα συνεχές δωδεκάμηνο.Ή τουλάχιστον ένα δίμηνο για μέχρι 29 ετών.") : skipToEnd("You must be unemployed for at least twelve months.");
      } else {
